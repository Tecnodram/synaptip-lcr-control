...
def _sort_plot(ax, x, y, **kw) -> None:
    idx = np.argsort(x)
    ax.plot(x[idx], y[idx], "o-", ms=4, linewidth=0.8,
            markeredgewidth=0.3, markeredgecolor="white", **kw)
...
def _plot_bode_grouped(ax, df, bias_groups, y_col, ylabel: str) -> None:
    if bias_groups:
        ...
        if len(f) > 0:
            idx = np.argsort(f)
            ax.plot(f[idx], y[idx], "o-", ms=4, color=_color(0),
                    linewidth=0.8, markeredgewidth=0.3, markeredgecolor="white")
