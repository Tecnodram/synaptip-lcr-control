...
Normalization contract (documentation only; no behavior change):
    - Coordinate conversion from magnitude/phase:
            Z_real = |Z| * cos(theta)
            Z_imag = |Z| * sin(theta)
    - Canonical columns are defined by COL_* and ANALYSIS_COLUMNS.
    - Domain validity filtering (freq > 0, finite values) is handled in
        cleaning_pipeline before analysis outputs are consumed.
    - Plot routines consume cleaned data and apply deterministic frequency sorting
        for stable model-aligned visualization.
    - DC-bias grouping is based on canonical dc_bias_v values when present.
"""
...
from analysis_engine.schema_detector import DetectedSchema
