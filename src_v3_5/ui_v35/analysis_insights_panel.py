...
class _AnalysisWorker(QThread):
    ...
    def _execute(self) -> None:
        ...
        from analysis_engine.eis_transformer import transform, COL_DC
        ...
        log("  Derived quantities computed.")
        ...
        group_col = (
            ...
            if (schema.has_dc_bias
                and COL_DC in enriched_df.columns
                and enriched_df[COL_DC].notna().any())
            else None
        )
        ...
        log(
            f"  Raw: {gs['points_raw']}  Clean: {gs['points_clean']}"
        ...
        if (
            "dc_bias_summary_tables" in self._selected
            and group_col
            and group_col in result.clean_df.columns
        ):
        ...
        non_plot = {"dc_bias_summary_tables", "smart_interpretation_report"}
        plot_selection = self._selected - non_plot
        gen = engine.run(
            result.clean_df,
            raw_df=enriched_df,
            selected=plot_selection if plot_selection else None,
        )
        ...
        if "smart_interpretation_report" in self._selected:
            ...
            interp_warnings = list(interp.warnings)
        ...
        # 9. Metadata
        # ------------------------------------------------------------------
        detected_warnings = list(schema.warnings) + interp_warnings + list(paths.errors)
        em.save_metadata(
            paths,
            run_config={
            ...
            timestamp=ts,
            input_filename=self._input_path.name,
            cleaning_summary={
                "global": gs,
                "per_group": result.summary.to_dict(orient="records"),
            },
            plots_generated=sorted(gen.keys()),
...
class AnalysisInsightsPanel(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._output_dir: Path = _app_base_dir() / "analysis_outputs"
    ...
    def _build_options_group(self) -> QGroupBox:
        ...
        categories = [
            ("Nyquist Plots",
             [k for k, _ in ANALYSIS_ITEMS if k.startswith("nyquist")]),
            ("Bode Plots",
             [k for k, _ in ANALYSIS_ITEMS if k.startswith("bode")]),
        ...
        for cat_title, keys in categories:
            ...
            cat_lbl.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
            cat_lbl.setStyleSheet("color: #374151; margin-top: 6px;")
    ...
    def _run_analysis(self) -> None:
        ...
        self._worker = _AnalysisWorker(
            self._input_path, out_dir, selected, parent=self
        )
