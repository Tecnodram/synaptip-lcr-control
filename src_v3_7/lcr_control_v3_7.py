"""
lcr_control_v3_7.py — SynAptIp Nyquist Analyzer V3.7 Entry Point
...
Run:
    cd src_v3_7
    python lcr_control_v3_7.py

Or from repo root:
    python src_v3_7/lcr_control_v3_7.py

V3.7 extends V3.6 with:
...
All V3.6, V3.5 and V3 functionality is preserved exactly as shipped.
"""
...
# Path resolution order: V3.7 first, then V3.6, then V3.5, then V3
_V3_7_DIR = (_BUNDLE_DIR / "src_v3_7") if getattr(sys, "frozen", False) else _HERE
...
# Desired sys.path order: [V3.7, V3.6, V3, V3.5, ...]
# Rationale:
#   - V3.7 first → wins for: ui_v37/
#   - V3.6 second → wins for: licensing/, ui_v36/
#   - V3   third  → wins for: services/, ui/   (V3 base package for ui.main_window_v3)
#   - V3.5 fourth → wins for: analysis_engine/, ui_v35/  (unique namespaces, no conflict)
# Inserting with sys.path[0:0] preserves the intended order directly.
sys.path[0:0] = [
    str(_V3_7_DIR),
    str(_V3_6_DIR),
    str(_V3_DIR),
...
_log(f"sys.path[0:5]: {sys.path[:5]}")

from PySide6.QtWidgets import QApplication

# V3.6 license system (inherited)
try:
...
# V3.7 main window
try:
    from ui_v37.main_window_v3_7 import MainWindowV3_7
except Exception:
    _log("Import failure for V3.7 UI module")
    _log(traceback.format_exc())
    raise


APP_NAME    = "SynAptIp Nyquist Analyzer V3.7"
APP_VERSION = "3.7.0"
APP_ORG     = "SynAptIp"
...
def main() -> int:
    ...
    try:
        _log("Creating V3.7 main window")
        window = MainWindowV3_7()
        window.show()
