...
set "SPEC_FILE=lcr_control_v4_1.spec"
set "EXE_NAME=SynAptIp_Nyquist_Analyzer_V4_1.exe"
set "APP_DIST_DIR=%DIST_DIR%\SynAptIp_Nyquist_Analyzer_V4_1"
...
echo.
echo ============================================================
echo  SynAptIp Nyquist Analyzer V4.1 -- Build Started
echo ============================================================
echo.

if exist "%BUILD_ROOT%\lcr_control_v4_1" rmdir /s /q "%BUILD_ROOT%\lcr_control_v4_1" >nul 2>&1
if exist "%DIST_DIR%\%EXE_NAME%" del /f /q "%DIST_DIR%\%EXE_NAME%" >nul 2>&1
if exist "%APP_DIST_DIR%" rmdir /s /q "%APP_DIST_DIR%" >nul 2>&1

for /d /r "%ROOT_DIR%src_v4_1" %%D in (__pycache__) do (
    if exist "%%D" rmdir /s /q "%%D" >nul 2>&1
)

"%PYTHON_EXE%" -m PyInstaller --noconfirm --clean ^
    --distpath "%DIST_DIR%" ^
    --workpath "%BUILD_ROOT%\lcr_control_v4_1" ^
    "%SPEC_FILE%"

if errorlevel 1 (
    echo.
    echo [ERROR] V4.1 build FAILED.
    exit /b 1
)
...
echo.
echo ============================================================
echo  SynAptIp Nyquist Analyzer V4.1 -- Build Complete
echo ============================================================
echo.
